  ...
  return (
    <main className="fexex-surface min-h-screen bg-[#161818] text-[#f4f3ee]">
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
        <Link href="/trade?type=crypto" className="text-sm font-semibold text-[#bfe3ff] hover:text-[#dbf1ff]">← All crypto assets</Link>
        {loading || status === "loading" ? <div className="mt-6 rounded-3xl border border-[#f4f3ee]/10 bg-[#202323] p-8 text-sm text-[#a9afa9]">Loading crypto trade...</div> : loadError || !crypto ? <section className="mt-6 rounded-3xl border border-red-400/20 bg-[#202323] p-8"><h1 className="text-2xl font-bold">Crypto asset unavailable</h1><p className="mt-2 text-sm text-red-200">{loadError || "This crypto asset was not found."}</p><Link href="/trade?type=crypto" className="mt-6 inline-flex rounded-xl bg-[#bfe3ff] px-4 py-3 font-bold text-[#161818]">Choose another asset</Link></section> : (
          <section className="fexex-pop-in mt-6 rounded-3xl border border-[#f4f3ee]/10 bg-[#202323] p-6 shadow-2xl shadow-black/30 sm:p-8 lg:p-10">
            <div className="flex items-center gap-4"><span className="flex h-16 w-16 items-center justify-center rounded-2xl border border-[#f4f3ee]/10 bg-[#f4f3ee] p-2 transition hover:-rotate-6 hover:scale-105"><img src={crypto.icon} alt={`${crypto.asset} icon`} className="h-full w-full object-contain" /></span><div><p className="text-sm font-semibold text-[#bfe3ff]">CRYPTO TO NAIRA WITHDRAWAL</p><h1 className="mt-1 text-3xl font-semibold">Sell {crypto.asset} <span aria-hidden="true">💎</span></h1></div></div>
            ...
            <form onSubmit={submitWithdrawal} className="mt-7 space-y-5">
              <div><label htmlFor="usd-value" className="mb-2 block text-sm font-medium text-[#d7dbd4]">{crypto.asset} value you are selling (USD)</label><input id="usd-value" type="number" inputMode="decimal" min="0.01" step="0.01" required value={usdValue} onChange={(event) => setUsdValue(event.target.value)} placeholder="e.g. 100" className="w-full rounded-xl border border-[#f4f3ee]/15 bg-[#1a1d1d] px-4 py-3 text-[#f4f3ee] outline-none placeholder:text-[#777a75] focus:border-[#bfe3ff] focus:ring-2 focus:ring-[#bfe3ff]/20" /></div>
              <div className="rounded-2xl border border-[#bfe3ff]/25 bg-[#bfe3ff]/10 p-4"><p className="text-xs font-semibold tracking-wide text-[#dbf1ff]">✦ LIVE NAIRA WITHDRAWAL ESTIMATE</p>{crypto.available ? <><p className="mt-2 text-sm text-[#d7dbd4]">{crypto.asset} pays {formatNaira(crypto.nairaPayoutPerUsd)} for every $1 of submitted value.</p><p className={`mt-2 text-2xl font-bold text-[#f4f3ee] transition ${estimatedPayout !== null ? "scale-105" : ""}`}>{estimatedPayout === null ? "Enter a crypto value" : formatNaira(estimatedPayout)}</p></> : <p className="mt-2 text-sm text-[#d7dbd4]">This asset is currently paused. You can return to choose another one.</p>}</div>
              ...
              {message && <p role="status" className="rounded-xl bg-[#bfe3ff]/10 px-4 py-3 text-sm text-[#dbf1ff]">{message}</p>}
              <button type="submit" disabled={submitting || !crypto.available || !defaultBankAccount} className="w-full rounded-xl bg-[#bfe3ff] px-4 py-3 font-bold text-[#161818] transition hover:-translate-y-0.5 hover:bg-[#dbf1ff] disabled:cursor-not-allowed disabled:translate-y-0 disabled:opacity-60">{submitting ? "Submitting withdrawal..." : `Request ${crypto.asset} withdrawal →`}</button>
            </form>
          </section>
        )}
      </div>
