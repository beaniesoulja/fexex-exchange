...
export default function TradeReceiptPage() {
  ...
  // A trade we just submitted already stashed everything the receipt needs, so
  // this can render immediately instead of showing a blocking loading screen
  // while the network request below quietly confirms the final version.
  const [receipt, setReceipt] = useState<Receipt | null>(() => (orderId ? readReceiptPreview(orderId) : null));
