...
function ActionButton({ children, danger = false, disabled = false, onClick }: { children: string; danger?: boolean; disabled?: boolean; onClick?: () => void }) {
  return <button type="button" onClick={onClick} disabled={disabled} className={`rounded-xl px-4 py-2.5 font-bold transition ${danger ? "bg-rose-500 text-white hover:bg-rose-600" : "bg-white text-[#1d2220] hover:bg-[#e6ebe3]"} disabled:cursor-not-allowed disabled:opacity-50`}>{children}</button>;
}
